...
import { lowerCaseInsensitivePath } from './common/pathUtils';
...
export class LensCollection<T> {
  private readonly data = new Map<string, { position: Position; dto: T }[]>();
  ...
  public getDtoAt(file: string, line: number): T | undefined {
    return this.data.get(lowerCaseInsensitivePath(file))?.find(p => p.position.line === line)?.dto;
  }
