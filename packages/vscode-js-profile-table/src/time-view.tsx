    ...
    (evt: KeyboardEvent, node: IGraphNode) => {
      ...
      switch (evt.key) {
        ...
        case 'ArrowLeft':
          ...
          break;
        case 'ArrowRight':
          if (node.children.size > 0 && !expanded.has(node)) {
        ...
        case '*':
          ...
          for (const child of focused?.parent?.children.values() || []) {
...
const TimeViewRow: FunctionComponent<{
  ...
  const expand = (
    <span className={styles.expander}>
      {node.children.size > 0 ? <Icon i={expanded.has(node) ? ChevronDown : ChevronRight} /> : null}
    </span>
  );
  ...
  return (
    <div
      ...
      aria-setsize={node.parent?.children.size ?? 1}
      aria-level={depth + 1}
